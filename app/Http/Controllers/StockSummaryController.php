    ...
    public function list(Request $request)
    {
        ...
        return response()->json([
            'items' => $paginator->getCollection()->map(fn ($item) => [
                ...
                'physical_quantity' => (float) ($item->physical_quantity ?? $item->quantity_on_hand),
                'saleable_quantity' => (float) ($item->saleable_quantity ?? $item->quantity_on_hand),
                'non_saleable_quantity' => (float) ($item->non_saleable_quantity ?? 0),
                'damaged_quantity' => (float) ($item->damaged_quantity ?? 0),
                'expired_quantity' => (float) ($item->expired_quantity ?? 0),
                'defective_quantity' => (float) ($item->defective_quantity ?? 0),
                'quarantined_quantity' => (float) ($item->quarantined_quantity ?? 0),
                'lost_quantity' => (float) ($item->lost_quantity ?? 0),
                ...
                'damaged_value' => round((float) ($item->damaged_value ?? 0), 2),
                'expired_value' => round((float) ($item->expired_value ?? 0), 2),
                'defective_value' => round((float) ($item->defective_value ?? 0), 2),
                'quarantined_value' => round((float) ($item->quarantined_value ?? 0), 2),
                'physical_value' => round((float) ($item->physical_value ?? $item->stock_value), 2),
