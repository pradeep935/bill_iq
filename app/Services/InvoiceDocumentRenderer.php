...
class InvoiceDocumentRenderer
{
    public function renderSalesInvoice(array $sale, bool $public = false): string
    {
        ...
        $itemRows = collect($sale['items'] ?? [])->values()->map(function ($item, $index) use ($money) {
            ...
            return '<tr>'
                ...
                . '<td><strong>' . e($item['product'] ?? 'Item') . '</strong><small>' . e($item['sku_snapshot'] ?? $item['barcode_snapshot'] ?? '') . '</small></td>'
        ...
        return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . e($sale['invoice_number']) . '</title>' . $this->invoiceCss() . '</head><body>' . $printAction . '<main class="invoice-page">'
            ...
            . '<section class="top-grid"><div><h2>' . e($business['name']) . '</h2><p>' . e($business['address']) . '</p><p>' . e($business['phone']) . ' <span>' . e($business['email']) . '</span></p><p>GSTIN: ' . e($business['gstin']) . '</p></div><dl><dt>Invoice No.</dt><dd>' . e($sale['invoice_number']) . '</dd><dt>Invoice Date</dt><dd>' . e($date) . '</dd><dt>Place of Supply</dt><dd>' . e($business['state']) . '</dd><dt>Payment Mode</dt><dd>' . e($paymentMode) . '</dd></dl></section>'
    ...
    {
        return '<style>
*{box-sizing:border-box}body{margin:0;background:#eef3f8;color:#061d3b;font-family:Arial,Helvetica,sans-serif}.invoice-page{width:1040px;max-width:100%;margin:0 auto;background:#fff;padding:30px 30px 0;box-shadow:0 18px 45px rgba(7,31,61,.14)}.action-bar{position:sticky;top:0;z-index:5;text-align:center;padding:10px;background:#eef3f8}.action-bar button{min-height:40px;padding:8px 18px;border:0;border-radius:8px;background:#082747;color:#fff;font-weight:800;cursor:pointer}.hero,.top-grid,.bill-to,.summary-layout,.footer-grid{display:grid;gap:22px}.hero{grid-template-columns:1fr auto;align-items:start}.brand{display:flex;align-items:center;gap:14px}.brand h1{margin:0;color:#082747;font-size:46px;line-height:1}.brand h1::after{content:"Q";color:#07835e}.brand p{margin:6px 0 0;text-transform:uppercase;color:#263f5d;font-size:11px;font-weight:800}.logo-mark{position:relative;width:72px;height:78px}.logo-mark span,.logo-mark b,.logo-mark i{position:absolute;bottom:10px;width:14px;background:#07835e;border-radius:8px 8px 0 0}.logo-mark span{left:12px;height:48px}.logo-mark b{left:30px;height:70px}.logo-mark i{left:48px;height:36px}.logo-mark::after{content:"";position:absolute;left:4px;bottom:0;width:64px;height:34px;border:10px solid #07835e;border-top:0;border-radius:0 0 44px 44px;transform:rotate(20deg)}.invoice-title{text-align:center}.invoice-title strong{display:block;padding:15px 30px;border-radius:8px;background:#082747;color:#fff;font-size:32px;letter-spacing:.5px}.invoice-title span{display:inline-block;margin-top:0;padding:9px 18px;border-radius:0 0 8px 8px;background:#ddf5ec;color:#061d3b;font-weight:800}.top-grid{grid-template-columns:1fr 1fr;margin-top:28px}.top-grid h2{margin:0 0 12px;font-size:28px;text-transform:uppercase}.top-grid p{margin:10px 0;color:#111827;font-size:15px;line-height:1.45}.top-grid dl{display:grid;grid-template-columns:150px 1fr;gap:14px 20px;margin:0;padding-left:44px;border-left:1px solid #c8d0da}.top-grid dt{color:#111827}.top-grid dd{margin:0;font-weight:800;color:#111827}.bill-to{position:relative;grid-template-columns:210px 1fr 260px;align-items:center;margin-top:24px;padding:20px;border:1px solid #cdd6e0;border-radius:10px;overflow:hidden}.bill-ribbon{margin-left:-20px;padding:24px 28px;border-radius:0 44px 44px 0;background:#082747;color:#fff;font-size:20px;font-weight:900}.bill-to h3{margin:0 0 10px;color:#111827}.bill-to p{margin:8px 0;color:#111827}.bag-art{height:112px;position:relative}.bag-art span,.bag-art b{position:absolute;bottom:0;border-radius:4px 4px 0 0}.bag-art span{right:80px;width:70px;height:92px;background:#8bddae}.bag-art b{right:32px;width:50px;height:76px;background:#07835e}.bag-art::before,.bag-art::after{content:"";position:absolute;border:3px solid #07835e;border-bottom:0;border-radius:20px 20px 0 0}.bag-art::before{right:96px;bottom:78px;width:28px;height:28px}.bag-art::after{right:42px;bottom:64px;width:22px;height:22px}.items{width:100%;border-collapse:separate;border-spacing:0;margin-top:14px;overflow:hidden;border-radius:8px}.items th{background:#006b52;color:#fff;font-size:12px}.items th:first-child{border-radius:8px 0 0 0}.items th:last-child{border-radius:0 8px 0 0}.items td,.items th{padding:12px 10px;border-right:1px solid #dde3ea;border-bottom:1px solid #dde3ea}.items td:first-child,.items th:first-child{border-left:1px solid #dde3ea}.items small{display:block;margin-top:3px;color:#637083}.center{text-align:center}.right{text-align:right}.strong{font-weight:900}.muted{color:#667085}.summary-layout{grid-template-columns:370px 1fr;margin-top:18px}.left-stack{display:grid;gap:18px;align-content:start}.info-card{border:1px solid #d8e0e8;border-radius:12px;padding:22px;background:#fff}.info-card h4,.footer-grid h4{margin:0 0 14px;color:#082747;text-transform:uppercase}.info-card p,.total-stack p{display:flex;justify-content:space-between;gap:16px;margin:12px 0}.info-card em{display:inline-block;margin-top:10px;padding:9px 18px;border-radius:8px;background:#ddf5ec;color:#0c7b56;font-style:normal;font-weight:900}.words{display:grid;grid-template-columns:54px 1fr;align-items:center}.coin{display:grid;place-items:center;width:52px;height:52px;border-radius:50%;background:#39a756;color:#fff;font-weight:900}.total-stack>p{padding-bottom:9px;border-bottom:1px dashed #c9d2dc;font-size:16px}.tax{width:100%;border-collapse:separate;border-spacing:0;margin:14px 0;border-radius:8px;overflow:hidden}.tax th{background:#138461;color:#fff}.tax td,.tax th{padding:9px;border:1px solid #dde3ea}.grand{display:flex;justify-content:space-between;align-items:center;margin-top:14px;padding:18px;border-radius:10px;background:#d9f7ed;color:#082747;font-size:22px;font-weight:900}.grand strong{font-size:34px;color:#02735a}.footer-grid{grid-template-columns:1fr 1fr 1.35fr;margin-top:24px;padding:22px;border:1px solid #d8e0e8;border-radius:12px}.footer-grid>div+div{border-left:1px solid #d8e0e8;padding-left:24px}.thanks{margin-top:22px;padding:14px;border-radius:0 8px 8px 0;background:#082747;color:#fff;font-weight:800}.qr{width:132px;height:132px;margin:10px auto;background:repeating-linear-gradient(45deg,#111 0 6px,#fff 6px 12px);border:12px solid #fff;box-shadow:0 0 0 1px #d7dfe8}.signature{margin-top:28px;text-align:right}.signature strong,.signature span{display:block}.signature::before{content:"";display:block;width:170px;margin:28px 0 8px auto;border-top:1px solid #94a3b8}footer{display:flex;justify-content:center;gap:38px;margin:26px -30px 0;padding:16px 30px;background:#082747;color:#fff;font-size:14px}@media(max-width:820px){.invoice-page{padding:18px 14px 0}.hero,.top-grid,.summary-layout,.footer-grid{grid-template-columns:1fr}.bill-to{grid-template-columns:1fr}.bill-ribbon{margin:0;border-radius:8px}.bag-art{display:none}.top-grid dl{padding-left:0;border-left:0}.footer-grid>div+div{border-left:0;border-top:1px solid #d8e0e8;padding-left:0;padding-top:16px}footer{display:grid;gap:8px;margin-left:-14px;margin-right:-14px}}@media print{body{background:#fff}.no-print{display:none}.invoice-page{width:100%;box-shadow:none;padding:14mm 10mm 0}.items td,.items th{padding:7px 8px}.footer-grid{break-inside:avoid}footer{position:fixed;left:0;right:0;bottom:0;margin:0}.brand h1{font-size:40px}.invoice-title strong{font-size:28px}}</style>';
    }
}
