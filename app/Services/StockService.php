...
class StockService
{
    ...
    {
        return DB::transaction(function () use ($data) {
            $this->productForScope($data)->newQuery()->whereKey($data['product_id'])->lockForUpdate()->firstOrFail();
    ...
    public function dashboard(array $filters = []): array
    {
        ...
        return [
            ...
            'expired_quantity' => round((float) $rows->sum('expired_quantity'), 3),
            'defective_quantity' => round((float) $rows->sum('defective_quantity'), 3),
            'quarantined_quantity' => round((float) $rows->sum('quarantined_quantity'), 3),
            'lost_quantity' => round((float) $rows->sum('lost_quantity'), 3),
