...
<template>
    ...
    <div :class="cls2" class="d-flex flex-wrap gap-2 mb-0" v-show="model.length > 0">
        <div class="multi-chip" :class="cls3" :style="'background-color: ' + color" v-for="opt in options" :key="optionValue(opt)" v-show="model.includes(optionValue(opt))">
    ...
    <div :class="`form-group ${cls}`">
        ...
        <select class="form-control" v-model="temp_select" :disabled="disabled" @change="newMultiSelectDisplay()">
            ...
            <option v-for="opt in options" :key="optionValue(opt)" :value="optionValue(opt)" v-show="!model.includes(optionValue(opt))">{{ optionLabel(opt) }}</option>
        </select>
