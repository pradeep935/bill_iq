...
<Layout :page="$props.page" :title="title">
    ...
    <CrudDrawer :model-value="modal" eyebrow="BATCH &amp; EXPIRY" save-label="Post Operation" :processing="saving" :save-disabled="!form.confirmed || (!receipt && !form.batch_id)" @save="submit" :title="operationNames[form.operation]" description="Post a controlled inventory operation with a document reference and reason." :errors="errors" @close="closeModal">
        <form v-if="modal" :key="form.operation" class="operation-form" @submit.prevent="submit">
            <div class="full operation-heading"><span>01</span><div><h2>Batch Information</h2><p>Choose the operation, batch and stock location.</p></div></div><FormSelect class="full" name="operation" label="Operation" :model-value="form.operation" :disabled="saving" :options="operations.map(([value,label]) => ({value,label}))" @update:model-value="value => value && openOperation(value)" />
            <template v-if="receipt"><FormSelect v-model="form.product_id" name="product_id" label="Product" :options="refs.products" opt_id="id" opt_name="name" select_name="Search product" :req="true" /><FormInput v-model="form.batch_number" name="batch_number" label="Batch Number"  :req="true" maxlength="100" /><DrawerField v-model="form.manufacturing_date" name="manufacturing_date" label="Manufacturing Date"  type="date" /><DrawerField v-model="form.expiry_date" name="expiry_date" label="Expiry Date"  type="date" :min="form.manufacturing_date || undefined" /><p class="full hint" v-if="form.operation==='stock_in'">For supplier purchases, use <a :href="localLink('/app/purchases')">Purchase Vouchers</a>. Manual Stock In posts through Stock Adjustment and its configured accounting rules.</p></template>
            <template v-else><template v-if="!selectedRow"><FormInput class="full" v-model="batchSearch" name="batchSearch" label="Search batch"  placeholder="Search batch or product" /><SearchSelect v-model="batchChoice" class="full" label="Batch and location" :options="operationBatchOptions" option-value-key="option_key" required @update:model-value="chooseBatch" /></template><div v-else class="selection full"><strong>{{ selectedRow.batch_number }} · {{ selectedRow.product_name }}</strong><span>{{ selectedRow.branch_name }} / {{ selectedRow.warehouse_name }} · {{ label(selectedRow.condition_status) }}</span><span>Physical {{ qty(selectedRow.quantity_on_hand) }} · Available {{ qty(selectedRow.quantity_available) }} · Reserved {{ qty(selectedRow.reserved_quantity) }}</span></div></template>
            <template v-if="!statusOnly"><template v-if="receipt"><FormSelect v-model="form.branch_id" name="branch_id" label="Branch" :options="refs.branches" :req="true" /><FormSelect v-model="form.warehouse_id" name="warehouse_id" label="Warehouse" :options="warehouses(form.branch_id)" :req="true" /><FormSelect v-model="form.condition_status" name="condition_status" label="Stock Condition" :options="refs.conditions.map(c => ({value: c, label: label(c)}))" :req="true" /></template>
            <FormSelect v-if="form.operation==='adjust'" v-model="form.direction" name="direction" label="Direction" :options="[{value:'in',label:'Adjustment In'},{value:'out',label:'Adjustment Out'}]" :req="true" /><FormInput v-model="form.quantity" name="quantity" label="Quantity"  type="number" min="0.001" step="0.001" :req="true" /><FormInput v-if="receipt || (form.operation==='adjust' && form.direction==='in')" v-model="form.unit_cost" name="unit_cost" label="Unit Cost"  type="number" min="0" step="0.01" :req="true" /><DrawerField v-model="form.document_date" name="document_date" label="Document Date"  type="date" required />
            <template v-if="form.operation==='transfer'"><FormSelect v-model="form.destination_branch_id" name="destination_branch_id" label="To Branch" :options="refs.branches" :req="true" /><FormSelect v-model="form.destination_warehouse_id" name="destination_warehouse_id" label="To Warehouse" :options="warehouses(form.destination_branch_id).filter(w=>Number(w.id)!==Number(form.warehouse_id))" :req="true" /></template>
            <FormSelect v-if="reclassifying" v-model="form.to_condition" name="to_condition" label="To Condition" :options="refs.conditions.filter(c=>c!==form.condition_status).map(c => ({value: c, label: label(c)}))" :req="true" /></template>
            ...
            <DrawerField class="full" v-model="form.reason" label="Reason / Notes" as="textarea" required :rows="3" :error="errors.reason?.[0]" hint="Explain why this stock operation is needed." /><label class="confirm full"><input v-model="form.confirmed" type="checkbox" /> I have checked the batch, location, quantity and reason.</label>
        </form>
    </CrudDrawer>
