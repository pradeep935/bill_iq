        ...
        <div class="inventory-control">
            ...
            <section v-if="!loading && tab === 'dashboard'" class="dashboard-stack">
                ...
                <section class="panel">
                    ...
                    <div class="table-wrapper"><table><thead><tr><th>Date & Time</th><th>Voucher No.</th><th>Operation</th><th>Product</th><th>Warehouse</th><th>Qty Change</th><th>User</th><th>Status</th></tr></thead><tbody><tr v-for="row in recentActivity" :key="row.id" @click="tab = 'register'"><td>{{ formatIndianDateTime(row.date) }}</td><td>{{ row.number || '-' }}</td><td>{{ labelize(row.type) }}</td><td>{{ row.raw?.product?.name || row.raw?.items?.[0]?.product?.name || '-' }}</td><td>{{ row.warehouse }}</td><td :class="['qty-change', row.action === 'transfer' ? 'neutral' : Number(row.quantity || 0) > 0 ? 'positive' : 'negative']">{{ row.action === 'transfer' ? qty(row.quantity) : signedQty(row.quantity) }}</td><td>{{ row.raw?.creator?.name || row.raw?.created_by?.name || row.raw?.approved_by?.name || '-' }}</td><td><span class="status-pill">{{ labelize(row.status) }}</span></td></tr><tr v-if="!recentActivity.length"><td colspan="8" class="empty">No stock movement history found.</td></tr></tbody></table></div>
                </section>
            </section>
