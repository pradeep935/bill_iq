...
const transferLineError = (item) => {
    ...
    if (Number(item.current_stock || 0) <= 0) return 'Out of stock at selected source warehouse.';
    ...
    <Layout :page="page" :title="title">
        ...
        <div class="inventory-control">
            ...
            <section v-if="tab === 'transfers'" class="transfer-page">
                ...
                <div class="transfer-action-bar"><button @click="resetTransferForm">Cancel</button><button :disabled="saving" @click="saveTransfer('draft')">Save Draft</button><button class="primary" :disabled="saving || !transfer.items.length" @click="saveTransfer(transfer.transfer_type === 'immediate' ? 'approved' : 'dispatched')">Post Transfer</button></div>

                <section class="panel transfer-history">
            ...
            <div v-if="transferDrawerOpen" class="drawer-backdrop" @click.self="closeTransferDrawer">
                <aside class="transfer-drawer">
                    ...
                    <div class="drawer-fields">
                        <label><span>Search Product</span><select v-model="transferItem.product_id" @change="refreshTransferLineStocks(transferItem)"><option value="">Search Product</option><option v-for="p in refs.products" :key="p.id" :value="p.id">{{ p.name }}</option></select></label>
                        <label><span>Available Stock</span><input :value="qty(transferItem.current_stock)" disabled /></label>
                        <label><span>Transfer Quantity</span><input v-model.number="transferItem.requested_quantity" type="number" min="0.001" step="0.001" @input="refreshTransferLineStocks(transferItem)" /></label>
                        <label><span>From Location</span><input v-model="transferItem.source_location" placeholder="From location" /></label>
                        <label><span>To Location</span><input v-model="transferItem.destination_location" placeholder="To location" /></label>
                        <label v-if="isBatchTracked(transferItem)"><span>Batch</span><input v-model="transferItem.source_batch_id" placeholder="Source batch" @change="refreshTransferLineStocks(transferItem)" /></label>
                        <label v-if="isSerialTracked(transferItem)"><span>Serial</span><input v-model="transferItem.source_serial_id" placeholder="Source serial" /></label>
                        <label><span>Unit Cost</span><input :value="money(transferItem.unit_cost)" disabled /></label>
                    </div>
                    <div class="drawer-preview"><div><span>Source</span><strong>{{ qty(transferItem.current_stock) }} -> {{ qty(safeSourceAfterTransfer(transferItem)) }}</strong></div><div><span>Destination</span><strong>{{ qty(transferItem.destination_stock) }} -> {{ qty(destinationAfterTransfer(transferItem)) }}</strong></div><div><span>Physical Stock</span><strong>{{ qty(transferPhysicalBefore(transferItem)) }} -> {{ qty(transferPhysicalAfter(transferItem)) }}</strong><small>No physical stock change</small></div></div>
                    <small v-if="transferLineError(transferItem)" class="line-error">{{ transferLineError(transferItem) }}</small>
                    <div class="actions"><button @click="closeTransferDrawer">Cancel</button><button class="primary" :disabled="Boolean(transferLineError(transferItem))" @click="addTransferItemFromDrawer">Add to Transfer</button></div>
                </aside>
            </div>
