        ...
        <div class="inventory-control">
            ...
            <section v-if="!loading && tab === 'dashboard'" class="dashboard-stack">
                ...
                <section class="panel">
                    ...
                    <div class="table-wrapper"><table><thead><tr><th>Date & Time</th><th>Voucher No.</th><th>Operation</th><th>Product</th><th>Warehouse</th><th>Qty Change</th><th>User</th><th>Status</th></tr></thead><tbody><tr v-for="row in recentActivity" :key="row.id" @click="tab = 'register'"><td>{{ formatIndianDateTime(row.date) }}</td><td>{{ row.number || '-' }}</td><td>{{ labelize(row.type) }}</td><td>{{ row.productName }}</td><td>{{ row.warehouse }}</td><td :class="['qty-change', row.action === 'transfer' ? 'neutral' : Number(row.quantity || 0) > 0 ? 'positive' : 'negative']">{{ row.action === 'transfer' ? qty(row.quantity) : signedQty(row.quantity) }}</td><td>{{ row.userName }}</td><td><span class="status-pill">{{ labelize(row.status) }}</span></td></tr><tr v-if="!recentActivity.length"><td colspan="8" class="empty">No stock movement history found.</td></tr></tbody></table></div>
                </section>
            </section>
            ...
            <section v-if="!loading && tab === 'voucher'" class="panel">
                ...
                <div v-for="(item, i) in adjustment.items" :key="i" class="line-grid adjustment-row"><select v-model="item.product_id" @change="refreshLineStock(item)"><option value="">Select Product</option><option v-for="p in refs.products" :key="p.id" :value="p.id">{{ p.name }} - {{ p.sku }}</option></select><input :value="item.product_id ? qty(item.current_stock) : ''" disabled placeholder="Previous Stock" /><select v-model="item.direction"><option v-for="option in movementOptions" :key="option.value" :value="option.value">{{ option.value === 'in' ? 'Increase' : 'Decrease' }}</option></select><input v-model.number="item.adjustment_quantity" type="number" step="0.001" /><input :value="item.product_id ? qty(lineNewStock(item)) : ''" disabled :class="lineNewStock(item) < 0 ? 'stock-negative' : ''" placeholder="New Stock" /><input v-model.number="item.unit_cost" type="number" step="0.01" /><input v-model="item.warehouse_location" placeholder="Location" /><select v-model="item.condition_status"><option v-for="option in conditionOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><button @click="adjustment.items.splice(i,1)" :disabled="adjustment.items.length === 1">Remove</button></div>
            ...
            <section v-if="!loading && tab === 'register'" class="panel">
                ...
                <div class="table-wrapper"><table><thead><tr><th>Voucher No</th><th>Type</th><th>Date</th><th>Branch</th><th>Warehouse</th><th>Product</th><th>Quantity</th><th>User</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="row in registerRows" :key="row.id"><td>{{ row.number }}</td><td>{{ labelize(row.type) }}</td><td>{{ formatIndianDateTime(row.date) }}</td><td>{{ row.branch }}</td><td>{{ row.warehouse }}</td><td>{{ row.productName }}</td><td>{{ qty(row.quantity) }}</td><td>{{ row.userName }}</td><td><span class="status-pill">{{ labelize(row.status) }}</span></td><td><button @click="tab = 'reports'; activeReport = 'movement_report'">Ledger</button><button v-if="permissions.approve && row.action === 'adjustment' && ['draft','submitted','approved'].includes(row.status)" @click="postAdjustment(row.raw)">Post</button><button v-if="permissions.approve && row.action === 'transfer' && ['approved','draft','submitted'].includes(row.status)" @click="dispatchTransfer(row.raw)">Dispatch</button><button v-if="permissions.approve && row.action === 'transfer' && ['dispatched','partially_received'].includes(row.status)" @click="receiveTransfer(row.raw)">Receive</button><button v-if="permissions.approve && row.action === 'count' && row.status !== 'posted'" @click="postVariance(row.raw)">Post Variance</button><button v-if="permissions.cancel && row.action === 'adjustment' && row.status === 'posted'" @click="reverseAdjustment(row.raw)">Cancel</button><button v-if="permissions.print" @click="printPage">Print</button></td></tr><tr v-if="!registerRows.length"><td colspan="10" class="empty">No inventory vouchers found.</td></tr></tbody></table></div>
            </section>
            ...
            <section v-if="tab === 'reasons'" class="panel">
                ...
                <div class="reason-form">
                    ...
                    <label><span>Movement Type</span><select v-model="reason.default_direction"><option v-for="option in movementOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>Determines whether this reason increases or decreases inventory.</small></label>
                    <label><span>Stock Condition</span><select v-model="reason.default_condition_status"><option v-for="option in conditionOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select><small>Describes why the stock changed or what condition the stock is in.</small></label>
            ...
            <section v-if="tab === 'reports'" class="panel">
                ...
                <div class="table-wrapper"><table><thead><tr><th>Date</th><th>Type</th><th>Product / Voucher</th><th>Branch</th><th>Warehouse</th><th>In</th><th>Out</th><th>Qty</th><th>Value</th><th>Status</th></tr></thead><tbody><tr v-for="(row, index) in currentReportRows" :key="row.id || index"><td>{{ formatIndianDateTime(row.transaction_date || row.adjustment_date || row.transfer_date || row.expiry_date) }}</td><td>{{ labelize(row.transaction_type || row.source || row.transfer_type || activeReport) }}</td><td>{{ row.product?.name || row.product_name || row.voucher_number || row.name || '-' }}</td><td>{{ row.branch?.name || row.branch_name || row.source_branch?.name || '-' }}</td><td>{{ row.warehouse?.name || row.warehouse_name || row.source_warehouse?.name || '-' }}</td><td>{{ qty(row.quantity_in) }}</td><td>{{ qty(row.quantity_out) }}</td><td>{{ qty(row.quantity_available || row.total_quantity_in || row.total_quantity_out) }}</td><td>Rs. {{ money(row.stock_value || row.total_value_in || row.total_value_out) }}</td><td>{{ labelize(row.status || row.stock_status || '-') }}</td></tr><tr v-if="!currentReportRows.length"><td colspan="10" class="empty">No report data found.</td></tr></tbody></table></div>
            </section>
