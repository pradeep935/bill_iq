...
const movementPagination = reactive({ current_page: 1, last_page: 1, per_page: 25, total: 0, from: 0, to: 0 });
...
const registerFilters = reactive({ search: '', reference_number: '', product_name: '', sku: '', barcode: '', movement_type: '', source_module: '', branch_id: '', warehouse_id: '', location: '', stock_condition: '', from_condition: '', to_condition: '', user_id: '', reason: '', posting_status: '', date_from: '', date_to: '', per_page: 25, page: 1, sort: 'date_time', direction: 'desc' });
...
const movementTypeOptions = [
    ...
    { value: 'opening_stock_reversal', label: 'Opening Stock Reversal' },
    { value: 'purchase', label: 'Purchase' },
    { value: 'sale', label: 'Sales Issue' },
    { value: 'sales_return', label: 'Sales Return' },
    { value: 'purchase_return', label: 'Purchase Return' },
    { value: 'stock_adjustment', label: 'Stock Adjustment' },
    { value: 'stock_reclassification', label: 'Stock Reclassification' },
    { value: 'stock_transfer', label: 'Stock Transfer' },
    { value: 'physical_count', label: 'Physical Count' },
    { value: 'location_transfer', label: 'Location Movement' },
...
const postingStatusOptions = [{ value: 'posted', label: 'Posted' }];
...
const insufficientStockLine = (item) => item.product_id && item.direction === 'out' && Number(item.adjustment_quantity || 0) > Number(item.current_stock || 0);
const insufficientStockMessage = (item) => `Insufficient stock. Available quantity is ${qty(item.current_stock || 0)}.`;
const adjustmentStockError = computed(() => {
...
const recentActivity = computed(() => (movementHistory.value.length ? movementHistory.value.map((row) => ({
    id: row.id,
    raw: row,
    ...
    physicalChange: row.physical_impact,
    movement: `${row.from_condition || '-'} -> ${row.to_condition || '-'}`,
    status: row.posting_status,
    action: 'ledger',
    productName: row.product || '-',
    userName: row.user || 'System',
})) : registerRawRows.value).slice().sort((a, b) => String(b.date || '').localeCompare(String(a.date || ''))).slice(0, 8));
...
const clearRegisterFilters = () => {
    Object.assign(registerFilters, { search: '', reference_number: '', product_name: '', sku: '', barcode: '', movement_type: '', source_module: '', branch_id: '', warehouse_id: '', location: '', stock_condition: '', from_condition: '', to_condition: '', user_id: '', reason: '', posting_status: '', date_from: '', date_to: '', page: 1, sort: 'date_time', direction: 'desc' });
    load();
};
const applyRegisterFilters = () => {
    registerFilters.page = 1;
    load();
};
...
const registerRows = computed(() => movementHistory.value);
const movementSort = (field) => {
    if (registerFilters.sort === field) {
        registerFilters.direction = registerFilters.direction === 'asc' ? 'desc' : 'asc';
    } else {
        registerFilters.sort = field;
        registerFilters.direction = 'desc';
    }
    registerFilters.page = 1;
    load();
};
const loadMovementPage = (page) => {
    if (page < 1 || page > movementPagination.last_page) return;
    registerFilters.page = page;
    load();
};
...
const load = async () => {
    ...
    try {
        ...
        const history = await InventoryApi.movementHistory(registerFilters);
        movementHistory.value = history.movements || [];
        Object.assign(movementPagination, history.pagination || {});
    } finally {
        loading.value = false;
    }
};
...
const exportMovementHistory = async (format) => {
    if (format === 'pdf') { window.print(); return; }
    const blob = await InventoryApi.exportMovementHistory(registerFilters, format);
    downloadBlob(blob, `movement-history.${format === 'excel' ? 'xls' : 'csv'}`);
};
const openLedgerDetail = (row) => { ledgerDetail.value = row; };
const closeLedgerDetail = () => { ledgerDetail.value = null; };
const exportRows = (format) => {
    ...
    downloadBlob(blob, `${activeReport.value}.${format === 'excel' ? 'xls' : 'csv'}`);
};
...
const isBatchTracked = (item) => Boolean(productFor(item.product_id).batch_required) || ['batch','batch_expiry','batch_serial'].includes(productFor(item.product_id).tracking_type);
const batchName = id => (refs.value.batches || []).find(b => Number(b.id) === Number(id))?.batch_no || '—';
const productBatches = item => (refs.value.batches || []).filter(b => Number(b.product_id) === Number(item.product_id));
        ...
        <div class="inventory-control">
            ...
            <section v-if="!loading && tab === 'voucher'" class="panel">
                ...
                <div v-for="(item, i) in adjustment.items" :key="i" class="line-grid adjustment-row">
                    ...
                    <label class="line-field"><span>Quantity</span><input v-model.number="item.adjustment_quantity" type="number" step="0.001" placeholder="Qty" :class="insufficientStockLine(item) ? 'stock-invalid' : ''" /><small v-if="insufficientStockLine(item)" class="line-error">{{ insufficientStockMessage(item) }}</small></label>
                ...
                <div v-if="adjustmentStockError" class="alert">{{ adjustmentStockError }}</div>
            ...
            <section v-if="!loading && tab === 'register'" class="panel movement-history-panel">
                <div class="section-head"><div><h2>Movement History</h2><p>Track every inventory movement across products, warehouses, stock conditions and source transactions.</p></div><div class="actions inline"><button @click="exportMovementHistory('csv')">CSV</button><button @click="exportMovementHistory('excel')">Excel</button><button @click="exportMovementHistory('pdf')">Print</button></div></div>
                <div class="voucher-help-card"><strong>Voucher Register reference</strong><p>The voucher register remains the header-level workflow for creating, posting and cancelling inventory vouchers. Movement History shows posted stock-ledger lines for audit and reconciliation.</p></div>
                <div class="form-grid filters movement-filters">
                    <input v-model="registerFilters.search" @keyup.enter="applyRegisterFilters" placeholder="Search voucher, product, SKU, barcode, reason or remarks" />
                    <input v-model="registerFilters.reference_number" @keyup.enter="applyRegisterFilters" placeholder="Voucher / reference no." />
                    <input v-model="registerFilters.product_name" @keyup.enter="applyRegisterFilters" placeholder="Product name" />
                    <input v-model="registerFilters.sku" @keyup.enter="applyRegisterFilters" placeholder="SKU" />
                    <input v-model="registerFilters.barcode" @keyup.enter="applyRegisterFilters" placeholder="Barcode" />
                    <select v-model="registerFilters.movement_type"><option value="">All Movement Types</option><option v-for="option in movementTypeOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                    <select v-model="registerFilters.source_module"><option value="">All Source Modules</option><option v-for="option in sourceModuleOptions" :key="option" :value="option">{{ option }}</option></select>
                    <select v-model="registerFilters.branch_id"><option value="">All Branches</option><option v-for="b in refs.branches" :key="b.id" :value="b.id">{{ b.name }}</option></select>
                    <select v-model="registerFilters.warehouse_id"><option value="">All Warehouses</option><option v-for="w in refs.warehouses" :key="w.id" :value="w.id">{{ w.name }}</option></select>
                    <input v-model="registerFilters.location" @keyup.enter="applyRegisterFilters" placeholder="Location" />
                    <select v-model="registerFilters.stock_condition"><option value="">All Conditions</option><option v-for="option in conditionOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                    <select v-model="registerFilters.from_condition"><option value="">From Condition</option><option v-for="option in conditionOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                    <select v-model="registerFilters.to_condition"><option value="">To Condition</option><option v-for="option in conditionOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                    <input v-model="registerFilters.user_id" @keyup.enter="applyRegisterFilters" placeholder="User ID" />
                    <input v-model="registerFilters.reason" @keyup.enter="applyRegisterFilters" placeholder="Reason" />
                    <select v-model="registerFilters.posting_status"><option value="">All Posting Statuses</option><option v-for="option in postingStatusOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                    <input v-model="registerFilters.date_from" type="date" />
                    <input v-model="registerFilters.date_to" type="date" />
                    <select v-model.number="registerFilters.per_page" @change="registerFilters.page = 1; load()"><option :value="25">25 rows</option><option :value="50">50 rows</option><option :value="100">100 rows</option></select>
                    <button @click="applyRegisterFilters">Apply</button>
                    <button @click="clearRegisterFilters">Clear</button>
                </div>
                <div class="table-wrapper sticky-register-table movement-table"><table><thead><tr><th @click="movementSort('date_time')">Date & Time</th><th>Reference</th><th>Source Module</th><th @click="movementSort('movement_type')">Movement Type</th><th>Product</th><th>SKU</th><th>Branch</th><th>Warehouse</th><th>From Location</th><th>To Location</th><th>From Condition</th><th>To Condition</th><th @click="movementSort('qty_in')">Qty In</th><th @click="movementSort('qty_out')">Qty Out</th><th @click="movementSort('net_quantity')">Net</th><th @click="movementSort('rate')">Rate</th><th @click="movementSort('movement_value')">Movement Value</th><th>User</th><th>Posting Status</th><th>Actions</th></tr></thead><tbody><tr v-for="row in registerRows" :key="row.id"><td>{{ formatIndianDateTime(row.date_time) }}</td><td>{{ row.reference_number || '-' }}</td><td>{{ row.source_module || '-' }}</td><td>{{ row.movement_type || '-' }}</td><td><strong>{{ row.product || '-' }}</strong><small>{{ row.barcode ? `Barcode: ${row.barcode}` : '' }}</small></td><td>{{ row.sku || '-' }}</td><td>{{ row.branch || '-' }}</td><td>{{ row.warehouse || '-' }}</td><td>{{ row.from_location || '-' }}</td><td>{{ row.to_location || '-' }}</td><td>{{ row.from_condition || '-' }}</td><td>{{ row.to_condition || '-' }}</td><td>{{ qty(row.qty_in) }}</td><td>{{ qty(row.qty_out) }}</td><td :class="['qty-change', Number(row.net_quantity || 0) > 0 ? 'positive' : Number(row.net_quantity || 0) < 0 ? 'negative' : 'neutral']">{{ signedQty(row.net_quantity) }}</td><td>Rs. {{ money(row.rate) }}</td><td>Rs. {{ money(row.movement_value) }}</td><td>{{ row.user || 'System' }}</td><td><span class="status-pill">{{ row.posting_status || 'Posted' }}</span></td><td><button @click="openLedgerDetail(row)">View</button><button v-if="permissions.print" @click="printPage">Print</button></td></tr><tr v-if="!registerRows.length"><td colspan="20" class="empty">No movement records found.</td></tr></tbody></table></div>
                <div class="movement-card-list"><article v-for="row in registerRows" :key="`card-${row.id}`" class="movement-card"><header><strong>{{ row.reference_number || 'Ledger Movement' }}</strong><span class="status-pill">{{ row.posting_status || 'Posted' }}</span></header><p>{{ row.movement_type }} | {{ row.source_module }}</p><b>{{ row.product || '-' }}</b><div><span>{{ formatIndianDateTime(row.date_time) }}</span><span>{{ row.warehouse || '-' }}</span></div><div><span>In {{ qty(row.qty_in) }}</span><span>Out {{ qty(row.qty_out) }}</span><span>Net {{ signedQty(row.net_quantity) }}</span></div><small>{{ row.impact_summary }}</small><button @click="openLedgerDetail(row)">View Details</button></article><div v-if="!registerRows.length" class="empty movement-empty">No movement records found.</div></div>
                <div class="movement-pagination"><span>Showing {{ movementPagination.from || 0 }}-{{ movementPagination.to || 0 }} of {{ movementPagination.total || 0 }}</span><div><button :disabled="movementPagination.current_page <= 1" @click="loadMovementPage(movementPagination.current_page - 1)">Previous</button><button :disabled="movementPagination.current_page >= movementPagination.last_page" @click="loadMovementPage(movementPagination.current_page + 1)">Next</button></div></div>
            </section>
            ...
            <div v-if="ledgerDetail" class="drawer-backdrop" @click.self="closeLedgerDetail">
                <aside class="transfer-drawer ledger-detail-drawer">
                    <div class="section-head drawer-head"><div><span>LEDGER LINE</span><h2>{{ ledgerDetail.reference_number || 'Movement Detail' }}</h2><p>{{ ledgerDetail.impact_summary }}</p></div><button class="icon-close" title="Close" @click="closeLedgerDetail">X</button></div>
                    <div class="ledger-detail-content">
                        <section><h3>Transaction</h3><dl><div><dt>Date & Time</dt><dd>{{ formatIndianDateTime(ledgerDetail.date_time) }}</dd></div><div><dt>Document Date</dt><dd>{{ formatReportDate(ledgerDetail.document_date) || '-' }}</dd></div><div><dt>Source Module</dt><dd>{{ ledgerDetail.source_module || '-' }}</dd></div><div><dt>Movement Type</dt><dd>{{ ledgerDetail.movement_type || '-' }}</dd></div></dl></section>
                        <section><h3>Product</h3><dl><div><dt>Product</dt><dd>{{ ledgerDetail.product || '-' }}</dd></div><div><dt>SKU</dt><dd>{{ ledgerDetail.sku || '-' }}</dd></div><div><dt>Barcode</dt><dd>{{ ledgerDetail.barcode || '-' }}</dd></div><div><dt>UOM</dt><dd>{{ ledgerDetail.uom || '-' }}</dd></div></dl></section>
                        <section><h3>Location And Stock</h3><dl><div><dt>Branch</dt><dd>{{ ledgerDetail.branch || '-' }}</dd></div><div><dt>Warehouse</dt><dd>{{ ledgerDetail.warehouse || '-' }}</dd></div><div><dt>Locations</dt><dd>{{ ledgerDetail.from_location || '-' }} -> {{ ledgerDetail.to_location || '-' }}</dd></div><div><dt>Conditions</dt><dd>{{ ledgerDetail.from_condition || '-' }} -> {{ ledgerDetail.to_condition || '-' }}</dd></div><div><dt>Quantity In</dt><dd>{{ qty(ledgerDetail.qty_in) }}</dd></div><div><dt>Quantity Out</dt><dd>{{ qty(ledgerDetail.qty_out) }}</dd></div><div><dt>Net Movement</dt><dd>{{ signedQty(ledgerDetail.net_quantity) }}</dd></div><div><dt>Physical Impact</dt><dd>{{ signedQty(ledgerDetail.physical_impact) }}</dd></div></dl></section>
                        <section><h3>Financial</h3><dl><div><dt>Rate</dt><dd>Rs. {{ money(ledgerDetail.rate) }}</dd></div><div><dt>Movement Value</dt><dd>Rs. {{ money(ledgerDetail.movement_value) }}</dd></div></dl></section>
                        <section><h3>Audit</h3><dl><div><dt>User</dt><dd>{{ ledgerDetail.user || 'System' }}</dd></div><div><dt>Posting Status</dt><dd>{{ ledgerDetail.posting_status || 'Posted' }}</dd></div><div><dt>Reason</dt><dd>{{ ledgerDetail.reason || '-' }}</dd></div><div><dt>Remarks</dt><dd>{{ ledgerDetail.remarks || '-' }}</dd></div></dl></section>
                    </div>
                    <div class="drawer-footer"><button @click="closeLedgerDetail">Close</button><button class="primary" @click="printPage">Print</button></div>
                </aside>
            </div>
            ...
            <div v-if="countDrawerOpen" class="drawer-backdrop" @click.self="closeCountDrawer">
                <aside class="transfer-drawer">
                    ...
                    <div class="drawer-fields">
                        ...
                        <label v-if="isBatchTracked(countItem)"><span>Batch</span><select v-model="countItem.batch_id" :disabled="!countItem.product_id" @change="refreshCountLineStock(countItem)"><option value="">Select batch</option><option v-for="b in productBatches(countItem)" :key="b.id" :value="b.id">{{ b.batch_no }}</option></select></label>
            ...
            <div v-if="transferDrawerOpen" class="drawer-backdrop" @click.self="closeTransferDrawer">
                <aside class="transfer-drawer">
                    ...
                    <div class="drawer-fields">
                        ...
                        <label v-if="isBatchTracked(transferItem)"><span>Source Batch</span><select v-model="transferItem.source_batch_id" :disabled="!transferItem.product_id" @change="transferItem.destination_batch_id=transferItem.source_batch_id; refreshTransferLineStocks(transferItem)"><option value="">Select batch</option><option v-for="b in productBatches(transferItem)" :key="b.id" :value="b.id">{{ b.batch_no }}</option></select></label>
                        <label v-if="isBatchTracked(transferItem)"><span>Destination Batch</span><input :value="batchName(transferItem.source_batch_id)" disabled /></label>
