...
const transferLineError = (item) => {
    ...
    if (Number(item.current_stock || 0) < transferQty(item)) return `Only ${qty(item.current_stock)} units available in source warehouse.`;
    ...
    <Layout :page="page" :title="title">
        ...
        <div class="inventory-control">
            ...
            <section v-if="tab === 'transfers'" class="panel">
                <div class="section-head"><div><h2>Stock Transfer</h2><p>Move inventory between branches and warehouses without changing total physical stock.</p></div></div>
                ...
                <div class="form-grid transfer-meta"><label><span>Transfer Date</span><input v-model="transfer.transfer_date" type="date" /></label><label><span>Transfer Type</span><select v-model="transfer.transfer_type"><option>immediate</option><option>dispatch_receive</option><option>inter_branch</option><option>inter_warehouse</option></select></label><label><span>Reference No.</span><input value="Auto generated" disabled /></label><label><span>Remarks</span><input v-model="transfer.remarks" placeholder="Remarks" /></label></div>
                <section class="transfer-items-card">
                    <div class="section-head"><div><h2>Transfer Items</h2><p>Add products with source availability and destination preview.</p></div><button class="primary" @click="addRow(transfer.items, { product_id: '', requested_quantity: 1, approved_quantity: '', unit_cost: 0, source_batch_id: '', destination_batch_id: '', source_serial_id: '', destination_serial_id: '', source_location: '', destination_location: '', current_stock: 0, destination_stock: 0 })">+ Add Transfer Item</button></div>
                    <div v-for="(item, i) in transfer.items" :key="i" class="transfer-item">
                        <div class="transfer-main-grid"><label class="product-field"><span>Product</span><select v-model="item.product_id" @change="refreshTransferLineStocks(item)"><option value="">Product</option><option v-for="p in refs.products" :key="p.id" :value="p.id">{{ p.name }}</option></select></label><label><span>Current Stock</span><input :value="qty(item.current_stock)" disabled /></label><label><span>Transfer Qty</span><input v-model.number="item.requested_quantity" type="number" min="0.001" step="0.001" @input="refreshTransferLineStocks(item)" /></label><label><span>Unit Cost</span><input v-model.number="item.unit_cost" type="number" step="0.01" /></label></div>
                        <div v-if="isBatchTracked(item) || isSerialTracked(item)" class="tracking-grid"><div v-if="isBatchTracked(item)" class="tracking-box"><strong>Batch Tracking</strong><label><span>Source Batch</span><input v-model="item.source_batch_id" placeholder="Source batch" @change="refreshTransferLineStocks(item)" /></label><label><span>Destination Batch</span><input v-model="item.destination_batch_id" placeholder="Destination batch" @change="refreshTransferLineStocks(item)" /></label></div><div v-if="isSerialTracked(item)" class="tracking-box"><strong>Serial Tracking</strong><label><span>Source Serial</span><input v-model="item.source_serial_id" placeholder="Source serial" /></label><label><span>Destination Serial</span><input v-model="item.destination_serial_id" placeholder="Destination serial" /></label></div></div>
                        <div class="transfer-main-grid location-grid"><label><span>From Location</span><input v-model="item.source_location" placeholder="From location" /></label><label><span>To Location</span><input v-model="item.destination_location" placeholder="To location" /></label></div>
                        <div class="transfer-preview"><div><span>Source Stock</span><strong>{{ qty(item.current_stock) }}</strong></div><div><span>Transfer</span><strong>{{ qty(transferQty(item)) }}</strong></div><div><span>Source After</span><strong>{{ qty(sourceAfterTransfer(item)) }}</strong></div><div><span>Destination Before</span><strong>{{ qty(item.destination_stock) }}</strong></div><div><span>Destination After</span><strong>{{ qty(destinationAfterTransfer(item)) }}</strong></div><div><span>Total Physical Stock</span><strong>{{ qty(transferPhysicalBefore(item)) }} -> {{ qty(transferPhysicalAfter(item)) }}</strong></div></div>
                        <div class="transfer-item-footer"><small v-if="transferLineError(item)" class="line-error">{{ transferLineError(item) }}</small><span v-else></span><button @click="transfer.items.splice(i,1)" :disabled="transfer.items.length === 1">Remove</button></div>
                    </div>
                </section>
                <div class="table-wrapper"><table><thead><tr><th>Product</th><th>Source</th><th>Destination</th><th>Available</th><th>Transfer Qty</th><th>Source After</th><th>Destination After</th><th>Batch/Serial</th><th>Cost</th><th>Action</th></tr></thead><tbody><tr v-for="(item, i) in transfer.items.filter((line) => line.product_id)" :key="`transfer-summary-${i}`"><td>{{ productFor(item.product_id).name || '-' }}</td><td>{{ optionLabel(refs.warehouses || [], transfer.source_warehouse_id, 'name') }}</td><td>{{ optionLabel(refs.warehouses || [], transfer.destination_warehouse_id, 'name') }}</td><td>{{ qty(item.current_stock) }}</td><td>{{ qty(transferQty(item)) }}</td><td>{{ qty(sourceAfterTransfer(item)) }}</td><td>{{ qty(destinationAfterTransfer(item)) }}</td><td>{{ item.source_batch_id || item.source_serial_id || '-' }}</td><td>Rs. {{ money(item.unit_cost) }}</td><td><button @click="transfer.items.splice(transfer.items.indexOf(item),1)" :disabled="transfer.items.length === 1">Remove</button></td></tr><tr v-if="!transfer.items.some((line) => line.product_id)"><td colspan="10" class="empty">No transfer items added.</td></tr></tbody></table></div>
                ...
                <div class="actions"><button :disabled="saving" @click="saveTransfer('draft')">Save Draft</button><button class="primary" :disabled="saving" @click="saveTransfer(transfer.transfer_type === 'immediate' ? 'approved' : 'dispatched')">Post Transfer</button></div>
