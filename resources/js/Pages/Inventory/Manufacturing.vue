...
const openBom = (row = null) => { current.value = row; resetBom(); if (row) bomForm.value = { ...bomForm.value, ...row, finished_product_id: row.finished_product_id, items: row.items?.map((i) => ({ ...i })) || bomForm.value.items }; modal.value = 'bom'; };
const openOrder = (row = null) => { current.value = row; resetOrder(); if (row) orderForm.value = { ...orderForm.value, ...row }; modal.value = 'order'; };
const saveBom = async () => { saving.value = true; try { await InventoryApi.saveBom(bomForm.value, current.value?.id || null); showToast('BOM saved.'); modal.value = null; await load(); await loadRefs(); } catch (e) { showToast(e?.response?.data?.message || 'BOM save failed.', 'error'); } finally { saving.value = false; } };
const saveOrder = async () => { saving.value = true; try { await InventoryApi.saveProductionOrder(orderForm.value, current.value?.id || null); showToast('Production order saved.'); modal.value = null; await load(); } catch (e) { showToast(e?.response?.data?.message || 'Order save failed.', 'error'); } finally { saving.value = false; } };
...
const completeOrder = async () => { saving.value = true; try { await InventoryApi.completeProductionOrder(current.value.id, completeForm.value); showToast('Production completed and posted.'); modal.value = null; await load(); } catch (e) { showToast(e?.response?.data?.message || 'Completion failed.', 'error'); } finally { saving.value = false; } };
...
<template>
    <Layout :page="page" :title="title">
        ...
        <InventoryModal v-if="modal === 'bom'" title="BOM" wide @close="modal = null">
        ...
        <InventoryModal v-if="modal === 'order'" title="Production Order" @close="modal = null">
        ...
        <InventoryModal v-if="modal === 'complete'" title="Complete Production" :subtitle="current?.order_number" wide @close="modal = null">
