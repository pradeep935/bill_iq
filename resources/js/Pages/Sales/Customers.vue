...
const actionMenuKey = (prefix, id) => `${prefix}-${id}`;
const toggleActionMenu = (target) => {
        ...
        <div class="sales-page">
            ...
            <section class="panel">
                <div class="form-grid">
                    ...
                    <label><span>Customer Name <span class="required-mark">*</span></span><input v-model="form.customer_name" placeholder="Customer Name" /></label>
                    <SearchSelect v-model="form.customer_type" label="Customer Type" :options="references.customer_types" option-value-key="value" option-label-key="label" select-placeholder="Select Type" required />
                    ...
                    <SearchSelect v-model="form.city" label="City" :options="filteredCities" option-value-key="name" option-label-key="name" select-placeholder="Search City" allow-custom />
                    ...
                    <SearchSelect v-model="form.status" label="Status" :options="[{ value: 'active', label: 'Active' }, { value: 'inactive', label: 'Inactive' }, { value: 'blocked', label: 'Blocked' }]" option-value-key="value" option-label-key="label" select-placeholder="Select Status" required />
            ...
            <section class="panel">
                <div class="toolbar">
                    ...
                    <SearchSelect v-model="stateId" label="State" :options="references.states" option-value-key="id" option-label-key="name" select-placeholder="All States" />
            ...
            <section v-if="detail" class="panel detail-panel">
                ...
                <div v-if="detail.recent_sales?.length" class="table-wrapper"><table><thead><tr><th>Invoice</th><th>Date</th><th>Total</th><th>Payment</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="row in detail.recent_sales" :key="row.id"><td>{{ row.invoice_number }}</td><td>{{ row.invoice_date }}</td><td>{{ formatMoney(row.grand_total) }}</td><td>{{ row.payment_status }}</td><td>{{ row.status }}</td><td><div class="row-actions"><RowActionMenu :open="openActionMenuId === actionMenuKey('invoice', row.id)" :show-view="false" more-label="Actions" more-title="Invoice actions" @toggle="toggleActionMenu(actionMenuKey('invoice', row.id))" @close="closeActionMenu"><button type="button" @click="openInvoicePrint(row.id); closeActionMenu()">View</button><a :href="`/app/sales/invoices/${row.id}/print`" target="_blank" @click="closeActionMenu">Print</a></RowActionMenu></div></td></tr></tbody></table></div>
