...
const closeView = () => { viewingSale.value = null; };
...
onUnmounted(() => { window.removeEventListener('popstate', fromUrl); window.removeEventListener('keydown', handlePreviewKeydown); });
</script>
    ...
    <Layout :page="props.page" :title="props.title">
        ...
        <div class="sales-page">
            ...
            <div v-if="viewingSale" class="invoice-modal-backdrop" @click.self="closeView">
                <section class="invoice-modal" role="dialog" aria-modal="true" :aria-label="`Invoice ${viewingSale.invoice_number}`">
                    ...
                    <div class="invoice-preview-frame">
                        <iframe :src="SalesApi.printUrl(viewingSale.id)" :title="`Invoice ${viewingSale.invoice_number}`"></iframe>
                    </div>
                </section>
            </div>
        </div>
...
.invoice-modal{grid-template-rows:auto auto minmax(0,1fr);width:min(1220px,97vw);height:min(900px,94vh);background:#f6f8fb}.invoice-modal-head{background:#fff!important;padding:14px 16px!important}.invoice-modal-title{min-width:0}.invoice-modal-title span{display:block}.invoice-modal-title h2{font-size:20px!important;line-height:1.1}.invoice-modal-title p{margin:4px 0 0;color:#69758a;font-size:12px;font-weight:750}.invoice-modal-actions button{min-height:36px;padding:8px 12px;border-radius:8px}.invoice-modal-actions .invoice-close{color:#d23f49;background:#fff3f4;border-color:#ffd6da}.invoice-modal-summary{display:grid;grid-template-columns:1.4fr repeat(3,minmax(150px,1fr));gap:10px;padding:12px 14px;background:#eef3f9;border-bottom:1px solid #dfe6ef}.invoice-modal-summary>div{min-width:0;padding:10px 11px;background:#fff;border:1px solid #dfe6ef;border-radius:8px}.invoice-modal-summary span{display:block;color:#69758a;font-size:10px;font-weight:850;text-transform:uppercase}.invoice-modal-summary strong{display:block;margin-top:4px;color:#142139;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.invoice-modal-summary small{display:block;margin-top:4px;color:#7a869a;font-size:11px;font-weight:750;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.invoice-modal-summary .badge{display:inline-flex;margin-top:2px}.invoice-preview-frame{min-height:0;margin:12px 14px 14px;overflow:hidden;background:#fff;border:1px solid #d8e0eb;border-radius:8px;box-shadow:inset 0 0 0 1px rgba(255,255,255,.6)}.invoice-preview-frame iframe{width:100%;height:100%;border:0;background:#fff}@media(max-width:900px){.invoice-modal-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.invoice-modal{height:100vh}.invoice-modal-title h2{font-size:17px!important}.invoice-modal-summary{grid-template-columns:1fr;padding:10px}.invoice-preview-frame{margin:10px;min-height:360px}.invoice-modal-actions{display:grid;grid-template-columns:1fr 1fr 1fr}.invoice-modal-actions button{width:100%;padding:8px 6px}}
</style>
