...
const totals = computed(() => {
    const total = form.items.reduce((sum, item) => {
        ...
        const sourceTotal = Number(item.line_total || 0);
        if (form.return_type === 'against_sale' && sold > 0 && sourceTotal > 0) return sum + (sourceTotal * qty / sold);
        return sum + qty * Number(item.selling_rate || 0);
    }, 0);
...
const printCreditNote = (row) => {
    window.open(SalesApi.salesReturnPrintUrl(row.id), '_blank');
};
const exportRows = () => {
    window.location.href = SalesApi.salesReturnExportUrl(filters);
};
    ...
    <Layout :page="page" :title="title">
        ...
        <div class="sales-page">
            ...
            <section class="panel">
                ...
                <div v-if="form.return_type === 'against_sale'" class="search-box"><input v-model="invoiceSearch" placeholder="Search invoice number, customer, mobile, GSTIN" @input="searchInvoices" /><div v-if="invoiceResults.length" class="search-results"><button v-for="invoice in invoiceResults" :key="invoice.id" @click="selectInvoice(invoice)"><strong>{{ invoice.invoice_number }}</strong><span>{{ invoice.customer }} | {{ invoice.invoice_date }} | {{ formatMoney(invoice.grand_total) }} | {{ invoice.returnable_status }}</span></button></div></div>
                ...
                <div class="table-wrapper"><table><thead><tr><th>Product</th><th>Variant</th><th>Batch</th><th>Sold</th><th>Returned</th><th>Available</th><th>Qty</th><th>Rate</th><th>Discount</th><th>GST</th><th>Condition</th><th>Restock</th><th>Reason</th><th></th></tr></thead><tbody><tr v-for="(item,index) in form.items" :key="`${item.product_id}-${index}`"><td><strong>{{ item.product }}</strong><span>{{ item.sku }}</span></td><td><span v-if="form.return_type==='against_sale'">{{ item.variant || '-' }}</span><select v-else v-model="item.product_variant_id"><option value="">Default</option><option v-for="v in item.variants || []" :key="v.id" :value="v.id">{{ v.sku || v.barcode }}</option></select></td><td><span v-if="form.return_type==='against_sale'">{{ item.batch || item.batch_id || '-' }} <small v-if="item.expiry_date">Exp {{ item.expiry_date }}</small></span><select v-else v-model="item.batch_id"><option value="">Batch</option><option v-for="b in item.batches || []" :key="b.id" :value="b.id">{{ b.batch_no }} | {{ b.expiry_date || '-' }}</option></select></td><td>{{ item.sold_quantity }}</td><td>{{ item.previously_returned }}</td><td>{{ item.available_quantity }}</td><td><input v-model="item.quantity" type="number" step="0.001" :max="item.available_quantity || undefined" /></td><td><input v-model="item.selling_rate" type="number" step="0.01" :disabled="form.return_type==='against_sale'" /></td><td><input v-model="item.discount_amount" type="number" step="0.01" :disabled="form.return_type==='against_sale'" /></td><td><input v-model="item.gst_rate" type="number" step="0.01" :disabled="form.return_type==='against_sale'" /></td><td><select v-model="item.condition_status"><option value="good">Resalable</option><option value="opened">Opened</option><option value="damaged">Damaged</option><option value="defective">Defective</option><option value="expired">Expired</option><option value="inspection_required">Inspection Required</option></select></td><td><select v-model="item.restock_status"><option value="restock">Restock to Saleable</option><option value="damaged_stock">Move to Damaged</option><option value="quarantine_stock">Move to Quarantine</option><option value="expired_stock">Move to Expired</option><option value="non_restockable">Do Not Restock</option></select></td><td><input v-model="item.return_reason" /></td><td><button class="danger" @click="removeItem(index)">Remove</button></td></tr><tr v-if="!form.items.length"><td colspan="14" class="empty">Select an invoice or add direct return products.</td></tr></tbody></table></div>
                ...
                <div class="total-grid"><span>Expected Credit <b>{{ formatMoney(totals.total) }}</b></span><span>Refund <b>{{ formatMoney(totals.refund) }}</b></span><span>Balance/Credit <b>{{ formatMoney(totals.balance) }}</b></span></div>
            ...
            <section class="panel"><div class="toolbar"><input v-model="filters.search" placeholder="Search credit note, invoice, customer" @input="loadReturns(1)" /><input v-model="filters.date_from" type="date" @change="loadReturns(1)" /><input v-model="filters.date_to" type="date" @change="loadReturns(1)" /><select v-model="filters.customer_id" @change="loadReturns(1)"><option value="">All Customers</option><option v-for="c in references.customers" :key="c.id" :value="c.id">{{ c.customer_name }}</option></select><select v-model="filters.branch_id" @change="loadReturns(1)"><option value="">All Branches</option><option v-for="b in references.branches" :key="b.id" :value="b.id">{{ b.name }}</option></select><select v-model="filters.warehouse_id" @change="loadReturns(1)"><option value="">All Warehouses</option><option v-for="w in references.warehouses" :key="w.id" :value="w.id">{{ w.name }}</option></select><select v-model="filters.status" @change="loadReturns(1)"><option value="">All Status</option><option value="draft">Draft</option><option value="approved">Approved</option><option value="cancelled">Cancelled</option><option value="reversed">Reversed</option></select><select v-model="filters.return_type" @change="loadReturns(1)"><option value="">All Types</option><option value="against_sale">Against Sale</option><option value="direct_return">Direct</option></select><select v-model="filters.settlement_type" @change="loadReturns(1)"><option value="">All Settlements</option><option value="customer_credit">Customer Credit</option><option value="cash_refund">Cash Refund</option><option value="upi_refund">UPI Refund</option><option value="pending">Pending</option></select><button @click="clearFilters">Clear Filters</button><button @click="exportRows">Export</button></div><div class="table-wrapper"><table><thead><tr><th>Credit Note</th><th>Date</th><th>Invoice</th><th>Customer</th><th>Branch</th><th>Warehouse</th><th>Type</th><th>Total</th><th>Refund</th><th>Balance</th><th>Settlement</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr v-for="row in returns" :key="row.id"><td>{{ row.credit_note_number }}</td><td>{{ row.return_date }}</td><td>{{ row.invoice_number || '-' }}</td><td>{{ row.customer || '-' }}</td><td>{{ row.branch || '-' }}</td><td>{{ row.warehouse || '-' }}</td><td>{{ row.return_type }}</td><td>{{ formatMoney(row.grand_total) }}</td><td>{{ formatMoney(row.refund_amount) }}</td><td>{{ formatMoney(row.balance_amount) }}</td><td>{{ row.settlement_type }}</td><td><span class="badge" :class="row.status">{{ row.status }}</span></td><td><div class="row-actions"><button @click="printCreditNote(row)">Print</button><button v-if="row.status==='draft'" @click="editReturn(row)">Edit</button><button v-if="row.status==='draft'" @click="simpleAction(SalesApi.approveSalesReturn,row,'Post return?')">Post</button><button v-if="row.status==='draft'" class="danger" @click="simpleAction(SalesApi.cancelSalesReturn,row,'Cancel draft?')">Cancel</button><button v-if="['approved','confirmed'].includes(row.status)" class="danger" @click="reverseReturn(row)">Reverse</button></div></td></tr><tr v-if="!returns.length && !loading"><td colspan="13" class="empty">No sales returns found for the selected filters.</td></tr></tbody></table></div><div v-if="pagination.total > 0 && pagination.last_page > 1" class="pagination"><button :disabled="pagination.current_page <= 1" @click="loadReturns(pagination.current_page - 1)">Previous</button><span>{{ pagination.from || 0 }}-{{ pagination.to || 0 }} of {{ pagination.total || 0 }}</span><button :disabled="pagination.current_page >= pagination.last_page" @click="loadReturns(pagination.current_page + 1)">Next</button></div></section>
        </div>
    </Layout>
</template>
