...
class SalesInventoryFlowTest extends TestCase
{
    ...
    public function test_fefo_sale_reconciles_all_batches_and_preserves_invoice_allocations(): void
    {
        ...
        foreach ([['MO-BATCH-001', '2026-12-31', 10], ['MO-BATCH-002', '2026-11-30', 5]] as [$number, $expiry, $quantity]) {
            $batch = \App\Models\ProductBatch::create(['business_id' => $businessId, 'product_id' => $product->id, 'batch_no' => $number, 'expiry_date' => $expiry, 'status' => 'active', 'condition_status' => 'saleable']);
